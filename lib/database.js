...
const ensureUserInDB = async(message, db, logger, args) => {
	return new Promise(function(resolve, reject) {
		logger.info('Check if user exists in database');
		asyncDbGet(db, 'SELECT * FROM users WHERE id = $id;', {$id: message.author.id})
			.then(
				(user) => {
				...
				() => {
					...
					asyncDbRun(db, 'INSERT INTO users(id) VALUES($id);', {$id: message.author.id})
						.then(() => {
